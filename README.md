...
### Dashboard Navigation & Capabilities:
1. **🏠 Home:** Project overview, dynamic cohort metrics, end-to-end methodology pipeline, and interpretable tree architecture preview.
2. **📊 Dataset Overview:** Cleveland dataset provenance, class balance breakdown, full 13-feature clinical taxonomy table, and interactive dataset viewer.
3. **📈 Healthcare Analytics:** Dynamic cohort filters (age range, sex, diagnosis) and 4 interactive analytical tabs (Demographics & Chest Pain, Physiological Biomarkers, Exercise Stress & Imaging, Pearson Correlation Heatmap).
4. **🎯 Model Performance:** 4-algorithm benchmark comparison table and chart, 5-Fold Stratified CV stability analysis, Baseline vs. Tuned Decision Tree metrics, Confusion Matrix, ROC Curve, and Decision Tree visualization.
5. **🩺 Risk Prediction:** Interactive patient parameter entry form, quick demonstration presets (Low-Risk, High-Risk, Reset), real-time risk classification badge ("Lower Predicted Risk" / "Higher Predicted Risk"), disease probability percentage, risk tier, contributing factor extraction, and medical safety notices.
...
6. **Run Automated Test Suite (34 unit tests):**
   ```bash
   pytest tests/ -v
   ```
